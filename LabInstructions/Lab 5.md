...
2.  Open the common-config-server and the common-eureka-server.  These are versions of what you created and used in the last few chapters.  If you haven't done so already, alter the common-config-server's application.yml to point to your own github repository.  Run the common-config-server, and run the common-eureka-server.
