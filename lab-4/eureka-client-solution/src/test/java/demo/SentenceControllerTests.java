...
public class SentenceControllerTests {
    ...
    public void testGetSentence() {
        ...
        when(discoveryClient.getInstances("LAB-4-SUBJECT")).thenReturn(Collections.singletonList(subjectInstance));
        when(discoveryClient.getInstances("LAB-4-VERB")).thenReturn(Collections.singletonList(verbInstance));
        when(discoveryClient.getInstances("LAB-4-ARTICLE")).thenReturn(Collections.singletonList(articleInstance));
        when(discoveryClient.getInstances("LAB-4-ADJECTIVE")).thenReturn(Collections.singletonList(adjectiveInstance));
        when(discoveryClient.getInstances("LAB-4-NOUN")).thenReturn(Collections.singletonList(nounInstance));
        ...
        String sentence = sentenceController.getSentence();
